...
export async function assembleEdit(
  ...
  if (local) {
    ...
    data.set('cameraSpeed', String(cameraSpeed));
    data.set('trimCameraEnd', String(trimCameraEnd));
  ...
  // Load the published ESM client as a static asset so Next.js does not rewrite
  // the worker's dynamic import of ffmpeg-core.
  const clientUrl = new URL('/ffmpeg/client/index.js', location.origin).href;
  const { FFmpeg } = (await import(
    /* webpackIgnore: true */ clientUrl
